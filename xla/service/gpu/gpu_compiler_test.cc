...
TEST_F(GpuCompilerTest, SymmetricBuffersMultipleCollectives) {
  constexpr absl::string_view kHloText = R"(
    ...
    ENTRY main {
      ...
      ag = f32[2048] all-gather(p0), replica_groups={{0,1}}, dimensions={0}, use_global_device_ids=true, channel_id=2
      ar_large = f32[2048] all-reduce(p1), replica_groups={}, to_apply=add, channel_id=3
      
      ROOT tuple = (f32[1024], f32[2048], f32[2048]) tuple(ar, ag, ar_large)
    }
  )";
