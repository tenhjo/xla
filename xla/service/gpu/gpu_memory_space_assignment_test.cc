...
TEST_F(GpuMemorySpaceAssignmentTest,
  ...
  absl::string_view kHloModule = R"(
    ...
    ENTRY main {
      ...
      ag = f32[2048]{0} all-gather(p0), replica_groups={{0,1}}, dimensions={0}, use_global_device_ids=true, channel_id=1
      ar_large = f32[2048]{0} all-reduce(p1), replica_groups={}, to_apply=add_f32

      ROOT tuple = (f32[1024]{0}, f32[2048]{0}, f32[2048]{0}) tuple(ar, ag, ar_large)
    }
  )";
  ...
  // ag (F32, 4096 bytes output, but operand is 4096 bytes) -> should be colored
  // as Collective
